    ...
    : IEmailSender
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        fromEmailAdress ??= optionsValue.FromEmailAddress;
        fromName ??= optionsValue.FromName;

        if (string.IsNullOrWhiteSpace(fromEmailAdress))
        {
        ...
        {
            From = new EmailAddress(fromEmailAdress, fromName ?? string.Empty),
            Subject = subject,
            HtmlContent = body,
        };
        ...
        if (!addedRecipients)
        {
            logger.LogWarning("No recipients were added to the email. At least one recipient (To, CC, BCC) is requried.");
            return false;
        }
        ...
        {
            logger.LogError("Email was not sent successfully. Status cpde: {StatusCode}", response.StatusCode);
            return false;
        }
